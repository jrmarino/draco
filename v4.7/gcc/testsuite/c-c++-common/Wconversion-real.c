...
/* Due to i386 rounding of long doubles on FreeBSD and DragonFly, the tests
   ...
   set both "target" and "xfail" together.  Rather than replace
   large_long_double target with xfail which would run it on all other
   platforms, the test is marked as UNSUPPORTED for these two. */
...
void h (void)
{
  ...
  fdouble (3.1L); /* { dg-warning "conversion" "" { target { ! "i?86-*-freebsd* i?86-*-dragonfly*" && large_long_double } } } */
  vdouble = 3.1L; /* { dg-warning "conversion" "" { target { ! "i?86-*-freebsd* i?86-*-dragonfly*" && large_long_double } } } */
